...
# version mirrors supported sdk
print('Version 0.45.0')
...
async def on_message(message):
    ...
    if message.content.startswith('$request') and message.channel.name in LISTENING_CHANNELS:
        ...
        print(requester.name, "funds request")
        ...
        if len(requester_address) != len(FAUCET_ADDRESS) or requester_address[:len(BECH32_HRP)] != BECH32_HRP:
            await channel.send(f'{requester.mention}, Invalid address format `{requester_address}`\n'
                               f'Address length must be equal to {len(FAUCET_ADDRESS)} and the prefix must be `{BECH32_HRP}`')
            return
